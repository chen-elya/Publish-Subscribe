...
public abstract class Subscriber extends RunnableSubPub {
    ...
    private  HashMap<Class,Callback> missions;
    /**
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        missions.putIfAbsent(type,callback);
    }
    ...
    public final void run() {
        initialize();
        //when the subscriber starts executing the run method, it registers itself with the MessageBroker and then calls the abstract initialize method
        MessageBrokerImpl.getInstance().register(this);
        while (!terminated) {
            try {
                Message m =  MessageBrokerImpl.getInstance().awaitMessage(this);//the subscriber fetch message from its message queue using the MessageBroker's 'awaitMessage'
                missions.get(m.getClass()).call(m);//for each message it should execute the corresponding callback
            }
